...
def e_step(cpt, conditional_data, data_in, parents):
    conditional_old = copy.deepcopy(conditional_data)
    yy = 0
    for row in data_in:
        xx = 0
            ...
            if element == -1:
                if parents[xx] == []:
                    ...
                    while aa < 2:
                        ...
                        # Iteratively consider all possible permutations of unknown values
                        while jj < math.pow(2, len(unknown_elements) - 1):
                            kk = 0
                            prob_unknown = 1.0
                            xval = return_binary_array(jj, len(unknown_elements))

                            # Iterate through unknown elemnts in row
                            while kk < len(unknown_elements):
                                if conditional_old[yy][unknown_elements[kk]] == -1:
                                    if isinstance(cpt[unknown_elements[kk]][xval[kk]], dict):
                                        pass
                                        # joint_prob = 0.0
                                        # for p_con in cpt[unknown_elements[kk]][xval[kk]].values():
                                        #     joint_prob += p_con
                                        # prob_unknown = prob_unknown * joint_prob
                                    else:
                                        prob_unknown = prob_unknown * cpt[unknown_elements[kk]][xval[kk]]
                                # elif parents[unknown_elements[kk]] == []:
                                else:
                                    prob_unknown = prob_unknown * conditional_old[yy][unknown_elements[kk]][xval[kk]]

                                kk += 1
                                # else:
                                #     pass
                            unknown[aa] += prob_unknown
                            jj += 1
                        aa += 1
                        conditional_data[yy][xx] = unknown

                #         ii = 0
                #         for element in row:
                #             jj = 0
                #             while jj < math.pow(2, len(unknown_elements)):
                #
                #
                #
                #             if element == -1:
                #                 if parents[ii] == []:
                #                     if conditional_data != -1:
                #                         unknown
                #                 else:
                #                     jj = 0
                #                     while jj < math.pow(2, len(parents[ii])):
                #                         prob_unknown = 1.0
                #                         xval = return_binary_array(jj, len(parents[ii]))
                #                         if conditional_data[yy][ii] != -1:
                #                             unknown = unknown * conditional_data[yy][ii][jj]
                #                         else:
                #                             unknown = unknown * conditional_
                #                         jj += 1
                #         px0 = (conditional_data[yy][xx][0] * cpt[xx][0]) / 15
                #         px1 = (conditional_data[yy][xx][1] * cpt[xx][1]) / 15
                #     else:
                #         px0 = cpt[xx][0]
                #         px1 = cpt[xx][1]
                #     conditional_data[yy][xx] = [px0, px1]
                # else:
